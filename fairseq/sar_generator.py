...
DecoderOut = namedtuple(
    "SARDecoderOut",
    ["output_tokens", "output_scores", "attn", "step", "max_step", "history", "output_slots"],
)


class SARGenerator(object):
    def __init__(
        self,
        tgt_dict,
        models=None,
    ...
    def generate(
        ...
        #####################################################
        # regenerate data based on length
        tgt_len = prev_output_tokens.size(1)
        length_order = (
            utils.new_arange(src_tokens, self.beam_size*tgt_len, bsz).t().reshape(-1)
        )
        # import pdb
        # pdb.set_trace()
        encoder_out = model.encoder.reorder_encoder_out(
            encoder_out, length_order
        )
        ...
        def finalized_hypos(step, prev_out_token, prev_out_score, prev_out_attn, prev_out_slot):
            cutoff = prev_out_token.ne(self.pad)
            # cutoff = prev_out_score.ne(0)
            # tokens = prev_out_token[cutoff]
            # if False in cutoff:
            # import pdb
            # pdb.set_trace()
            tokens = torch.cat([torch.cat([prev_out_slot[j-4], torch.tensor([6]).to(prev_out_token)]) for j in prev_out_token[cutoff]])
        ...
        for step in range(self.max_iter + 1):
            ...
            # decoder_out = autoregressive_decode(model, prev_decoder_out, encoder_out, src_tokens, **decoder_options)
            ...
            finalized_slots = decoder_out.output_slots
            if len(finalized_slots) > max_list:
                max_list = len(finalized_slots)
            if self.retain_history:
